...
def TCA_eval(basepath, matfile, matvar, outputfilename):
    ...
    dataspike_slice = np.squeeze(dataspike[:, :, :, np.where(np.logical_and(time >= -0.2, time <= 0.5))])
...
if __name__ == '__main__':
    ...
    TCA_eval(basepath=basepath, matfile=matfile, matvar=matvar, outputfilename=outputfilename)
...
